...
class MazeScene extends Phaser.Scene {
  ...
  showToast(msg, color = "#00ff00") {
    ...
    this.toast = this.add.text(
      this.cameras.main.centerX, this.cameras.main.height - 40, msg,
  ...
  create() {
    ...
    while (placed < orbsCount) {
      ...
      if (this.mazeLayout[ry][rx] === 1 && !(rx === this.entrance.x && ry === this.entrance.y)) {
        ...
        const orb = this.add.circle(px, py, 6, 0x00ff66);
        orb.setStrokeStyle(2, 0x33ffaa, 1);
        ...
        // pulse
        const pulse = this.add.circle(px, py, 8, 0x00ff66, 0.25);
        pulse.setDepth(orb.depth - 1);
        orb.pulse = pulse;
        orb.pulseTween = this.tweens.add({
          targets: pulse, scale: 1.8, alpha: 0, duration: 1200, ease: "Sine.out",
          yoyo: false, repeat: -1,
    ...
    // Optional joystick (if present in HTML)
    this.hookJoystick();
  ...
  // ---------- Joystick handlers (optional) ----------
  hookJoystick() {
    const root = document.getElementById("joystick");
    if (!root) return;

    this.mobileDir = { up:false, down:false, left:false, right:false };
    const setDir = (dir, val) => { this.mobileDir[dir] = val; };

    const bind = (selector, dir) => {
      const el = root.querySelector(selector);
      if (!el) return;
      const onDown = (e) => { e.preventDefault(); setDir(dir, true); };
      const onUp   = (e) => { e.preventDefault(); setDir(dir, false); };
  ...
  update() {
    ...
    const left  = this.cursors.left.isDown  || (this.mobileDir && this.mobileDir.left);
    const right = this.cursors.right.isDown || (this.mobileDir && this.mobileDir.right);
    const up    = this.cursors.up.isDown    || (this.mobileDir && this.mobileDir.up);
    const down  = this.cursors.down.isDown  || (this.mobileDir && this.mobileDir.down);
  ...
  findPath() {
    ...
    while(q.length){
      ...
      if (c.x===end.x && c.y===end.y){
        const path=[]; let cur=c; while(cur){ path.unshift(cur); cur=prev[key(cur.x,cur.y)]; }
        return path;
      }
  ...
  handleReset(){
    ...
    saveGameSnapshot({
      currentLevel: sameLevel, totalTime: this.totalTime, score: this.score, coins: this.coins,
      runSeed: sameRunSeed, rngSeed: sameLevelSeed, playerName: PLAYER_NAME
    });
    this.scene.restart({
      currentLevel: sameLevel, totalTime: this.totalTime, score: this.score, coins: this.coins,
      runSeed: sameRunSeed, rngSeed: sameLevelSeed, snapshot: null
    });
  }
  ...
  nextLevel(){
    ...
    this.currentLevel++;
    saveGameSnapshot({
      currentLevel: this.currentLevel, totalTime: this.totalTime, score: this.score, coins: this.coins,
      runSeed: this.runSeed, rngSeed: makeLevelSeed(this.runSeed, this.currentLevel), playerName: PLAYER_NAME
    });
...
// --- Fit canvas to available area (desktop layout) ---
function fitCanvas() {
  const canvas = document.getElementById("gameCanvas");
  if (!canvas || !game || !game.scale) return;

  // prendi lo spazio utile del contenitore centrale
  const container = canvas.parentElement; // .stage-center
  const maxW = Math.min(container.clientWidth || window.innerWidth, 1280);
  const maxH = Math.min((container.clientHeight || (window.innerHeight - 160)), 800);

  game.scale.resize(maxW, maxH);
  // rifletto anche sul DOM (utile quando si usa un <canvas> pre-esistente)
