...
class MazeScene extends Phaser.Scene{
  ...
  updateHUD(){
    ...
    if (mT) mT.textContent = this.timerStarted ? this.formatTime(Date.now() - this.startTime) : '00:00:00';
  }
  ...
  create(){
    ...
    // Bottoni UI mobile (HUD)
    withBtn('btnSaveM',  function(){ this.handleSave();  this.showToast('Saved');  });
    withBtn('btnLoadM',  function(){ this.handleLoad();  this.showToast('Loaded'); });
    withBtn('btnResetM', function(){ this.handleReset(); this.showToast('Level reset'); });
    withBtn('btnHintM',  function(){ this.useHint(); });
  ...
  hookJoystick(){
    ...
    const bind = (selector, dir) => {
      const el = joy.querySelector(selector);
      if (!el) return;
