...
// --- Save system con namespace per nickname ---
function saveKey() {
...
// --- Hint tuning ---
const HINT_COST = 3; // costo in sfere per usare un hint
...
// --- Effetto glow ---
const GLOW_COLOR = 0xff99ff; // più chiaro del fucsia
const glowWidths = [10, 5, 2];       // spessori vari
const glowAlphas = [0.15, 0.35, 0.8]; // trasparenze variabili
...
this.events.on('update', () => {
  ...
  glowWidths.forEach((w, i) => {
    ...
    // Sfere energetiche (ENERGY)
    this.energyGroup = this.physics.add.group();
    const rngForOrbs = mulberry32((this.rngSeed >>> 0) + 999);
    const sizeForOrbs = size;
    const orbsCount = Math.max(3, 3 + Math.floor(this.currentLevel / 2)); // cresce lentamente
    let placed = 0;
    while (placed < orbsCount) {
      const rx = Math.floor(rngForOrbs() * sizeForOrbs);
      const ry = Math.floor(rngForOrbs() * sizeForOrbs);
      // Metti solo su celle corridoio e non sopra l'entrata
      // Metti solo su celle corridoio e non sopra l'entrata
if (this.mazeLayout[ry][rx] === 1 && !(rx === this.entrance.x && ry === this.entrance.y)) {
  ...
  // ==== VERSIONE BASE (riempimento + bordo) ====
  // Cambia qui i colori:
...
placed++;
  ...
  useHint(){
    ...
    // === Effetto HINT con Glow ===
    const glowColors = [0x33ffaa, 0x00ff66, 0x00ff66]; // lime → verde brillante → verde scuro
...
for (let i = 0; i < glowColors.length; i++) {
  ...
  // dissolvenza per ogni livello di glow
  this.tweens.add({
    targets: g,
    alpha: 0,
    duration: 3000,
...
this.showToast("Hint used!", "#00ffff");
  ...
  nextLevel(){
    ...
    if (this.currentLevel > 20) {
      ...
      // --- Schermata finale: pannello + testi, niente sovrapposizioni ---
const cx = this.cameras.main.centerX;
const cy = this.cameras.main.centerY;

// Container per tenere insieme tutto
...
// Pannello scuro dietro (copre il labirinto, non i testi)
...
// Ordine: prima pannello, poi testi
endContainer.add([panel, title, stats]);
    ...
    // reset timer e restart scena per il livello successivo
    this.isGameOver   = false;
    this.timerStarted = false;
    this.startTime    = 0;
    const timerEl = document.getElementById('timer-display');
