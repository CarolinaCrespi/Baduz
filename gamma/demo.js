/* =========================================================
   ...
   ---------------------------------------------------------
   + Supporto controlli touch (D-Pad)
   ========================================================= */
...
/* =============
   SALVATAGGI
   ============= */
function saveKey() { return `${SAVE_VERSION_PREFIX}${PLAYER_NAME}`; }
...
class MazeScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let row = 0; row < size; row++) {
      for (let col = 0; col < size; col++) {
        const x = col * this.tileSize; const y = row * this.tileSize;
    ...
    // Heuristica: dispositivo touch?
    this.isMobile = this.sys.game.device.input.touch || window.matchMedia('(hover: none), (pointer: coarse)').matches;
    ...
    while (placed < orbsCount) {
      ...
      if (this.mazeLayout[ry][rx] === 1 && !(rx === this.entrance.x && ry === this.entrance.y)) {
        ...
        orb.pulseTween = this.tweens.add({
          ...
          onRepeat: () => { pulse.setScale(1); pulse.setAlpha(ORB_PULSE_OPACITY); }
        });
    ...
    this.physics.add.overlap(this.ant, this.energyGroup, (ant, orb) => {
      ...
      orb.destroy();
      this.coins++; this.showToast('+1 Orb', '#00ffff'); this.updateHUD();
    }, null, this);
    ...
    /* ---- Reazioni ai cambi di stato per i controlli touch ---- */
    const mobileControls = document.getElementById('mobile-controls');
    this.events.on('pause',  () => mobileControls && (mobileControls.style.opacity = '0.2'));
    this.events.on('resume', () => mobileControls && (mobileControls.style.opacity = '1'));
  ...
  update() {
    ...
    // Lettura input: tastiera + D-Pad
    const p = (window.baduzPad || {up:false,down:false,left:false,right:false});
    const c = this.cursors;

    let vx = 0, vy = 0;
    if (p.left  || (c.left  && c.left.isDown))  vx -= 1;
    if (p.right || (c.right && c.right.isDown)) vx += 1;
    if (p.up    || (c.up    && c.up.isDown))    vy -= 1;
    if (p.down  || (c.down  && c.down.isDown))  vy += 1;

    // Normalizza diagonale
    if (vx !== 0 && vy !== 0){ const k = Math.SQRT1_2; vx *= k; vy *= k; }

    const SPEED = this.isMobile ? 180 : 200; // touch leggermente più “morbido”
    this.ant.body.setVelocity(vx * SPEED, vy * SPEED);
  ...
  findPath() {
    ...
    const key = (x,y)=>`${x},${y}`;
    const q=[start]; const seen = new Set([key(start.x,start.y)]); const prev={};
    while (q.length) {
      const c = q.shift();
      ...
      for (const [dx,dy] of dirs) {
        ...
        if (nx>=0 && ny>=0 && nx<size && ny<size && this.mazeLayout[ny][nx]===1 && !seen.has(key(nx,ny))) {
          seen.add(key(nx,ny)); prev[key(nx,ny)]=c; q.push({x:nx,y:ny});
        }
      }
    }
    return null;
  ...
  handleReset() {
    const sameLevel = this.currentLevel; const sameRunSeed = this.runSeed; const sameLevelSeed = makeLevelSeed(sameRunSeed, sameLevel);
    saveGameSnapshot({ currentLevel: sameLevel, totalTime: this.totalTime, score: this.score, coins: this.coins, runSeed: sameRunSeed, rngSeed: sameLevelSeed, playerName: PLAYER_NAME });
    this.scene.restart({ currentLevel: sameLevel, totalTime: this.totalTime, score: this.score, coins: this.coins, runSeed: sameRunSeed, rngSeed: sameLevelSeed, snapshot: null });
  }
  ...
  nextLevel() {
    ...
    this.currentLevel++;
    saveGameSnapshot({ currentLevel: this.currentLevel, totalTime: this.totalTime, score: this.score, coins: this.coins, runSeed: this.runSeed, rngSeed: makeLevelSeed(this.runSeed, this.currentLevel), playerName: PLAYER_NAME });

    if (this.currentLevel > 20) {
      this.isGameOver = true; this.cameras.main.stopFollow();
      ...
      const cx = this.cameras.main.centerX; const cy = this.cameras.main.centerY;
      const endContainer = this.add.container(cx, cy).setScrollFactor(0).setDepth(2000);

      const title = this.add.text(0, -35, `DEMO COMPLETED!\nThank you for playing, ${PLAYER_NAME}!`, { fontSize: '40px', fill: '#ffffff', fontFamily: 'Arial', align: 'center' }).setOrigin(0.5);
      const stats = this.add.text(0, 60, `Total Time: ${formattedTotal}\nScore: ${this.score}`, { fontSize: '24px', fill: '#00ff00', fontFamily: 'Arial', stroke: '#000', strokeThickness: 2, align: 'center' }).setOrigin(0.5);
      ...
      this.physics.pause(); this.ant.body.setVelocity(0);
      return;
    }

    // Reset timer e avvia livello seguente
    this.isGameOver = false; this.timerStarted = false; this.startTime = 0;
    const timerEl = document.getElementById('timer-display'); if (timerEl) timerEl.textContent = '00:00:00';

    this.scene.restart({ currentLevel: this.currentLevel, totalTime: this.totalTime, score: this.score, coins: this.coins, runSeed: this.runSeed, rngSeed: makeLevelSeed(this.runSeed, this.currentLevel) });
  }
  ...
  generatePrimMaze(maze, rng) {
    const size = maze.length; let walls = [];
    const possibleCells = [];
    for (let y = 1; y < size - 1; y += 2) for (let x = 1; x < size - 1; x += 2) possibleCells.push({ x, y });
    ...
    while (walls.length > 0) {
      ...
      if (maze[cy][cx] === 0) {
        maze[wy][wx] = 1; maze[cy][cx] = 1; this.addWalls(cx, cy, maze, walls);
      }
    }
  }
  ...
  placeEntranceExit(maze, rng) {
    const size = maze.length; const isTopBottom = rng() < 0.5;
    let start, end;
...
const config = {
  ...
  scene: MazeScene,
  // Fit e centratura: utile soprattutto in demo su mobile
  scale: {
